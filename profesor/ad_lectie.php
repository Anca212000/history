...
<div class="rotund">
    ...
    if(@$_POST['modif_lect'])
    {
        if($text==NULL or $fisier==NULL)
        {
			...
			<input type=\"hidden\" name=\"ncapt\" value=\"$nume\">
        ...
        {
            $k=1;
            $lectie=mysql_query("SELECT * FROM `lectii` WHERE `id-cap`='$valoare'");
            while($rand=mysql_fetch_row($lectie))
                if($text==$rand[2])
                    $k=0;
            if(!$k)
            ...
            else
            {
				...
				if( $extensie === 'php')
				{
					...
					@rename("capitole/$nncapt[0]/$text/".$fisier,"capitole/$nncapt[0]/$text/$text.php");
