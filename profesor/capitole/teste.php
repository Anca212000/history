...
<?php
$d=$_SESSION['domeniu'];
echo "<span><a class=\"ac\" href=\"$ncapt/index.php\">Acasa</a></span>";
?>
<section>
<?php
if($d=='profesor')
...
for($r=1;$r<=6;$r++)
{
	...
	$testi=$rand1[2];
	echo"<tr><td><div class=\"cifre\"><b>$r</b></div>&nbsp;&nbsp;&nbsp;&nbsp;</td><td>$testi</td></tr>";
...
<form method='post' action='teste.php'>
   	<?php
		$ss=1;
		$punct=0;
		$cap1=$_SESSION['numecap'];
$rez22=@mysql_query("SELECT * FROM capitole WHERE `nume-cap`='$cap1'");
$ncap=@mysql_fetch_array($rez22);
        $rez1=mysql_query("SELECT * FROM intrebari WHERE `id-cap`='$ncap[0]'" );
        $nr_rand1=@mysql_num_rows($rez1);
        if($nr_rand1==0)
        	echo "<h3 style=\"color:maroon;padding-top:20%;float:left;\">Nu există întrebări la acest capitol.</h3>";
        else
        {
  	for($mm=1;$mm<=$nr_rand1;$mm++)
		{$inregistrare=mysql_fetch_array($rez1);
		 $tab_in[$mm]=$inregistrare[0];
		}               
		for($l=1;$l<=6;$l++)
        {
			$ok=0;
            ...
            $rezi=@mysql_query("SELECT * FROM intrebari WHERE `id-intreb`='$tab_in[$i]' ");
